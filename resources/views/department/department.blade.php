...
{{--                            @if (Auth::user()->type == DataClass::USER_ADMIN || Auth::user()->type == DataClass::USER_SYSTEM)--}}
{{--                                <div class="col-md-1 col-sm-12 col-xs-12">--}}
{{--                                    <a title="Add New User" class="nav navbar-right btn btn-info"--}}
...
{{--                            @endif--}}
                        ...
                        <div class="x_content">
                            <table id="datatable-responsive"
                                ...
                                <tr>
                                    <th>#</th>
                                    <th><span style="color: darkgray">Department Name</span></th>
                                ...
                                @if(count($departments) > 0)
                                    ...
                                    @foreach($departments as $key=>$department)
                                        <tr>
                                            <th scope="row">{{@++$sl}}</th>
                                            <th scope="row">{{$department->label}}</th>
                                            ...
                                            <td>
                                                <a title="Edit" class="btn btn-sm btn-primary"
                                                   href="#editModal{{$department->id}}" data-toggle="modal">
                                                    <i class="fa fa-edit"></i>
                                                </a>

                                                <a data-trash-id="{{$department->id}}"
    ...
    <form id="trash-form" action="{{url('department/trash')}}" method="POST" style="display: none;">
        @csrf
